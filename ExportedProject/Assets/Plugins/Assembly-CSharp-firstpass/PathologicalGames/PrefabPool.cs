...
namespace PathologicalGames
{
	...
	public class PrefabPool
	{
		...
		internal void SelfDestruct()
		{
			if (logMessages)
			{
				Debug.Log($"SpawnPool {spawnPool.poolName}: Cleaning up PrefabPool for {prefabGO.name}...");
			}
		...
		public Transform SpawnNew(Vector3 pos, Quaternion rot)
		{
			...
			Transform transform = spawnPool.InstantiatePrefab(prefabGO, pos, rot).transform;
		...
		private IEnumerator PreloadOverTime()
		{
			...
			int remainder = num % preloadFrames;
			int numPerFrame = num / preloadFrames;
			forceLoggingSilent = true;
		...
		public bool Contains(Transform transform)
		{
			if (prefabGO == null)
			{
				Debug.LogError($"SpawnPool {spawnPool.poolName}: PrefabPool.prefabGO is null");
			}
