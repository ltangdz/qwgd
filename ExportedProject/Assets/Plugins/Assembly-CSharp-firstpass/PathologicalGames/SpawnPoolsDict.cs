	...
	public class SpawnPoolsDict : IDictionary<string, SpawnPool>, ICollection<KeyValuePair<string, SpawnPool>>, IEnumerable<KeyValuePair<string, SpawnPool>>, IEnumerable
	{
		...
		public void AddOnCreatedDelegate(string poolName, OnCreatedDelegate createdDelegate)
		{
			if (!onCreatedDelegates.ContainsKey(poolName))
			{
				...
				Debug.Log($"Added onCreatedDelegates for pool '{poolName}': {createdDelegate.Target}");
			}
			else
			{
				Dictionary<string, OnCreatedDelegate> dictionary = onCreatedDelegates;
				dictionary[poolName] = (OnCreatedDelegate)Delegate.Combine(dictionary[poolName], createdDelegate);
			}
		}
		...
		public void RemoveOnCreatedDelegate(string poolName, OnCreatedDelegate createdDelegate)
		{
			...
			Dictionary<string, OnCreatedDelegate> dictionary = onCreatedDelegates;
			dictionary[poolName] = (OnCreatedDelegate)Delegate.Remove(dictionary[poolName], createdDelegate);
			Debug.Log($"Removed onCreatedDelegates for pool '{poolName}': {createdDelegate.Target}");
		}
		...
		public void DestroyAll()
		{
			...
			{
				Debug.Log("DESTROYING: " + pool.Value.gameObject.name);
