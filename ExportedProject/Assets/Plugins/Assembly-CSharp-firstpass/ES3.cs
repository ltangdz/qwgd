...
public static class ES3
{
	...
	public static byte[] Serialize<T>(T value, ES3Settings settings = null)
	{
		...
		using (MemoryStream memoryStream = new MemoryStream())
		{
			using (ES3Writer eS3Writer = ES3Writer.Create(memoryStream, settings, writeHeaderAndFooter: false, overwriteKeys: false))
			{
	...
	public static void CopyFile(ES3Settings oldSettings, ES3Settings newSettings)
	{
		...
		{
			throw new InvalidOperationException(string.Concat("Cannot copy file from ", oldSettings.location, " to ", newSettings.location, ". Location must be the same for both source and destination."));
		}
	...
	public static void RenameFile(ES3Settings oldSettings, ES3Settings newSettings)
	{
		if (oldSettings.location != newSettings.location)
		{
			throw new InvalidOperationException(string.Concat("Cannot rename file in ", oldSettings.location, " to ", newSettings.location, ". Location must be the same for both source and destination."));
		}
		if (oldSettings.location == Location.File)
		{
			if (ES3IO.FileExists(oldSettings.FullPath))
