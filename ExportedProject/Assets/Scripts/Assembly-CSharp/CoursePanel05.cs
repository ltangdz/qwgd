...
public class CoursePanel05 : MonoBehaviour
{
	...
	public GameObject noteitemname;

	public GameObject noteitembirth;
	...
	{
		DeleteHighLight(btn_sql, isneedclick: true);
	...
	public void Next()
	{
		if (iscanclick)
		{
			if (pos == 0)
			{
				AddHighLight(noteitemname);
				AddHighLight(noteitembirth);
				noteitemname.GetComponent<NoteItem>().SetHighLightItem(ihl: true);
				noteitembirth.GetComponent<NoteItem>().SetHighLightItem(ihl: true);
			}
			else if (pos == 1)
			{
				DeleteHighLight(noteitemname);
				DeleteHighLight(noteitembirth);
				noteitemname.GetComponent<NoteItem>().SetHighLightItem(ihl: false);
				noteitembirth.GetComponent<NoteItem>().SetHighLightItem(ihl: false);
			}
			else if (pos == 2)
			{
				HideBlack();
	...
	private IEnumerator StartHideBlack()
	{
		...
		AddHighLight(btn_sql, isneedclick: true);
		MoveArrow();
	}
	...
	private IEnumerator StartHideCourse(bool isactive)
	{
		...
		img_bottom.transform.DOLocalMove(new Vector3(0f, -634f, 0f), 0.2f);
		yield return new WaitForSeconds(0.3f);
		gameManager.player.playerdata.isCourse05 = 1;
		if (isactive)
		{
			gameManager.CanShowSetting(-1);
