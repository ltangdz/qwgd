...
public class DanielVideoDialog : MonoBehaviour
{
	...
	public void Init(string dataid)
	{
		...
		gameManager.musicManager.LowerVol();
		if (gameManager.dataManager.dic39.ContainsKey(dataid))
		{
	...
	{
		Object.Instantiate(Resources.Load<GameObject>(endLoadPrefab), gameManager.homeScene.middle);
		if (endLoadPrefab.IndexOf("yulun") > -1)
		{
	...
	private void CioSaying(int i)
	{
		if (!isSaying)
		{
			...
			gameManager.soundManager.Stop();
			float num = 0f;
			if (zimus.Length >= 1 && yuyin.Length >= 1)
			{
				num = gameManager.soundManager.PlayEventFinished("110004", int.Parse(yuyin[i].Split(':')[1]));
