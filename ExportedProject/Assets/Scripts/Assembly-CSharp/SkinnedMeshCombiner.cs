...
public class SkinnedMeshCombiner : MonoBehaviour
{
	...
	private void Start()
	{
		...
		for (int j = 0; j < m_SpriteMeshInstances.Length; j++)
		{
			...
			{
				SkinnedMeshRenderer skinnedMeshRenderer = spriteMeshInstance2.cachedSkinnedRenderer;
				BoneWeight[] boneWeights = spriteMeshInstance2.sharedMesh.boneWeights;
				for (int k = 0; k < boneWeights.Length; k++)
				{
					BoneWeight item = boneWeights[k];
				...
				item3.transform = skinnedMeshRenderer.localToWorldMatrix;
				list3.Add(item3);
		...
		skinnedMeshRenderer2.sharedMesh.bindposes = list4.ToArray();
		skinnedMeshRenderer2.sharedMesh.RecalculateBounds();
		skinnedMeshRenderer2.materials = spriteMeshInstances[0].sharedMaterials;
