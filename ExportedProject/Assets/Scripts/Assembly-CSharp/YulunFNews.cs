...
public class YulunFNews : MonoBehaviour, IBeginDragHandler, IEventSystemHandler, IDragHandler, IEndDragHandler
{
	public float[,] pos;
	...
	{
		running = true;
		GameObject gameObject = Object.Instantiate(Resources.Load<GameObject>("Dialog/Yulun/yulun_zfile"), base.transform);
		filesList.Add(gameObject);
		if (pos.GetLength(0) >= filesList.Count)
		{
			...
			gameObject.GetComponent<YulunZfile>().Init(newsInfo);
			running = false;
		}
	}
	...
	public void OnBeginDrag(PointerEventData eventData)
	{
		if (base.transform.childCount > 0 && !running)
		{
			...
			YulunZfile component = filesList[filesList.Count - 1].GetComponent<YulunZfile>();
	...
	public void OnEndDrag(PointerEventData eventData)
	{
		if (base.transform.childCount > 0 && !running)
		{
			...
			if (gameObject != null)
			{
				gameObject.GetComponent<YulunNewsChoiceBox>().AddNewFiles(filesList[filesList.Count - 1].GetComponent<YulunZfile>().newsInfo);
