...
public class CoursePanel07 : MonoBehaviour
{
	...
	public GameObject noteitememail;

	public GameObject noteitememai2;
	...
	{
		DeleteHighLight(btn_email, isneedclick: true);
	...
	public void Next()
	{
		if (iscanclick)
		{
			if (pos == 0)
			{
				noteitememail.GetComponent<NoteItem>().SetHighLightItem(ihl: true);
				noteitememai2.GetComponent<NoteItem>().SetHighLightItem(ihl: true);
				AddHighLight(noteitememail);
				AddHighLight(noteitememai2);
			}
			else if (pos == 1)
			{
				DeleteHighLight(noteitememail);
				DeleteHighLight(noteitememai2);
				noteitememail.GetComponent<NoteItem>().SetHighLightItem(ihl: false);
				noteitememai2.GetComponent<NoteItem>().SetHighLightItem(ihl: false);
				HideBlack();
			}
			ShowText();
		}
	...
	private IEnumerator StartHideBlack()
	{
		...
		AddHighLight(btn_email, isneedclick: true);
		MoveArrow();
	}
	...
	private IEnumerator StartHideCourse(bool isactive)
	{
		...
		img_bottom.transform.DOLocalMove(new Vector3(0f, -634f, 0f), 0.2f);
		yield return new WaitForSeconds(0.3f);
		gameManager.player.playerdata.isCourse07 = 1;
		if (isactive)
		{
			gameManager.CanShowSetting(-1);
