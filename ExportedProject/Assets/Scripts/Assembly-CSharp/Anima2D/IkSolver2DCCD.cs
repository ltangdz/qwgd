...
namespace Anima2D
{
	...
	public class IkSolver2DCCD : IkSolver2D
	{
		...
		protected override void DoSolverUpdate()
		{
			...
			Vector3 vector2 = base.rootBone.transform.InverseTransformPoint(targetPosition);
			...
			for (int j = 0; j < iterations; j++)
			{
				for (int num2 = base.solverPoses.Count - 1; num2 >= 0; num2--)
				{
					...
					Vector3 b = vector2 - solverPose2.solverPosition;
					Vector3 a = vector - solverPose2.solverPosition;
					b.z = 0f;
					a.z = 0f;
					Quaternion quaternion = Quaternion.AngleAxis(MathUtils.SignedAngle(a, b, Vector3.forward) * num, Vector3.forward);
