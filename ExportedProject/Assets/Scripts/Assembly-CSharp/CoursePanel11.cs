...
public class CoursePanel11 : MonoBehaviour
{
	...
	{
		DeleteHighLight(btn_summit, isneedclick: true);
	}
	...
	public void Next()
	{
		if (iscanclick)
		{
			if (pos == 0 && btn_summit != null)
			{
				btn_summit.GetComponent<Button>().interactable = true;
				AddHighLight(btn_summit, isneedclick: true);
			}
			Debug.Log("Next");
			ShowText();
		}
	...
	{
		Debug.Log("ready add:" + highlightobject);
		if (!(highlightobject == null) && !(highlightobject.GetComponent<Canvas>() != null))
		{
			Debug.Log("1ready add:" + highlightobject);
			...
			Debug.Log("2ready add:" + highlightobject);
		}
	}
	...
	private IEnumerator StartHideCourse(bool isactive)
	{
		...
		gameManager.player.playerdata.isCourse11 = 1;
		if (isactive)
		{
			gameManager.CanShowSetting(-1);
