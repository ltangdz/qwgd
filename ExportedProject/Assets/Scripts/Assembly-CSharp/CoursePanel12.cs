...
public class CoursePanel12 : MonoBehaviour
{
	...
	{
		DeleteHighLight(sqlitem, isneedclick: true);
	}
	...
	public void Next()
	{
		if (iscanclick)
		{
			if (pos == 0)
			{
				AddHighLight(sqlitem, isneedclick: true);
			}
			ShowText();
		}
	}
	...
	private IEnumerator StartHideCourse(bool isactive)
	{
		...
		gameManager.player.playerdata.isCourse12 = 1;
		if (gameManager.homeScene.sqlDialog != null)
		{
			gameManager.homeScene.sqlDialog.img_drag.gameObject.SetActive(value: true);
		}
		if (isactive)
		{
			gameManager.CanShowSetting(-1);
