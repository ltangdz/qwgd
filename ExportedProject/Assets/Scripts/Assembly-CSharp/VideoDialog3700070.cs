...
public class VideoDialog3700070 : MonoBehaviour
{
	...
	public void ClickZimu()
	{
		...
		if (pos < (float)zimus.Length)
		{
			if (!isSaying)
			{
				...
				Debug.Log("pos:" + biaoqing[(int)pos]);
				van.ShowExpression(biaoqing[(int)pos]);
				StopAllCoroutines();
		...
		else if (pos >= (float)zimus.Length && !hundown)
		{
			...
			gameManager.soundManager.Stop();
			van.ShowExpression(biaoqing[(int)pos - 1]);
			hundown = true;
			txt_zimu2.text = "";
			...
			Object.Instantiate(Resources.Load<GameObject>("zhadan/zhadaninvade1"), homeScene.middle);
		}
	}
