...
public class VideoDialog3710002 : MonoBehaviour
{
	...
	public void ClickZimu()
	{
		...
		if (pos < (float)zimus.Length)
		{
			if (!isSaying)
			{
				...
				{
					pos += 1f;
					if (pos == 2f || pos == 4f || pos == 6f || pos == 8f || pos == 11f || pos == 14f)
					{
						pos -= 0.5f;
					}
					isSaying = false;
		...
		else if (pos >= (float)zimus.Length && !hundown)
		{
			...
			if (!string.IsNullOrEmpty(_missonId))
			{
				gameManager.homeScene.goalDialog.CompleteItem(_missonId);
				gameManager.player.playerdata.reasoninglist.Add("4012");
			}
	...
	public void HideVideoDialog()
	{
		...
		gameManager.homeScene.goalDialog.CompeleteOneItem("2010002");
		gameManager.homeScene.ShowCatchButton(1);
