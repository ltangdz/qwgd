...
public class HackerVideoDialog02 : MonoBehaviour
{
	...
	public HackerUploadDialog hackerUploadDialog;
	...
	public void ClickSelect(int poss)
	{
		if (selectGroup.iscanclick)
		{
			...
			iscanclick = true;
			pos += 0.5f;
			if (!gameManager.player.playerdata.cioAnwser.ContainsKey(zimus[(int)pos - 1]))
			{
				gameManager.player.playerdata.cioAnwser.Add(zimus[(int)pos - 1], poss);
			}
			else
			{
				gameManager.player.playerdata.cioAnwser[zimus[(int)pos - 1]] = poss;
			}
