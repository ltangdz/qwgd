...
{
	public class DDOSBulletDLC8 : AlubaSpawnDLC8
	{
		...
		private void OnTriggerEnter2D(Collider2D other)
		{
			...
			EventManagerDlc8.NoticeSound(GetSoundType());
			if (other.gameObject.name == "EnmenyWall")
			{
				EventManagerDlc8.NoticeWallInjured(_damage, isEnemy: true, null);
			}
			else
			{
				EnemyDLC8 component = other.gameObject.GetComponent<EnemyDLC8>();
		...
		public void Shoot(BulletType bulletType, Transform target, float speed, int damage, bool isFlood)
		{
			...
			InitUI();
			EventManagerDlc8.NoticeSound(DdosSound.SHOOT);
		...
		private void InitUI()
		{
			...
			bulletImage.sprite = DdosManagerDlc8.ddosAtlas.GetSprite(text);
		}
