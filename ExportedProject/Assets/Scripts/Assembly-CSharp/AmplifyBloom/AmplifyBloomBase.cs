	...
	public class AmplifyBloomBase : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			if (m_finalCompositionShader != null)
			{
				...
				if (!m_finalCompositionMaterial.GetTag(AmplifyUtils.ShaderModeTag, searchFallbacks: false).Equals(AmplifyUtils.ShaderModeValue))
				{
		...
		private void OnRenderImage(RenderTexture src, RenderTexture dest)
		{
			...
			if (m_highPrecision)
			{
				AmplifyUtils.EnsureKeywordEnabled(m_bloomMaterial, AmplifyUtils.HighPrecisionKeyword, state: true);
				AmplifyUtils.EnsureKeywordEnabled(m_finalCompositionMaterial, AmplifyUtils.HighPrecisionKeyword, state: true);
				AmplifyUtils.CurrentRTFormat = RenderTextureFormat.DefaultHDR;
			}
			else
			{
				AmplifyUtils.EnsureKeywordEnabled(m_bloomMaterial, AmplifyUtils.HighPrecisionKeyword, state: false);
				AmplifyUtils.EnsureKeywordEnabled(m_finalCompositionMaterial, AmplifyUtils.HighPrecisionKeyword, state: false);
