...
public class VideoDialog3700060 : MonoBehaviour
{
	...
	public void ClickZimu()
	{
		...
		if (pos < (float)zimus.Length)
		{
			if (!isSaying)
			{
				...
				if (zimus.Length >= 1)
				{
					num = gameManager.soundManager.PlayEventFinished("110004", int.Parse(yuyin[(int)pos].Split(':')[1]));
