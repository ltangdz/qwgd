...
public class VideoDialog3710001 : MonoBehaviour
{
	...
	public void ClickZimu()
	{
		...
		if (pos < (float)zimus.Length)
		{
			if (!isSaying)
			{
				...
				{
					pos += 1f;
					if (pos == 3f || pos == 5f || pos == 8f || pos == 11f)
					{
						pos -= 0.5f;
					}
					isSaying = false;
		...
		else if (pos >= (float)zimus.Length && !hundown)
		{
			...
			if (!string.IsNullOrEmpty(_missonId))
			{
				...
				gameManager.player.playerdata.reasoninglist.Add("4011");
			}
	...
	public void HideVideoDialog()
	{
		...
		gameManager.homeScene.goalDialog.CompeleteOneItem("2010001");
