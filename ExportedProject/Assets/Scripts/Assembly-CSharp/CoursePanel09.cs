...
public class CoursePanel09 : MonoBehaviour
{
	...
	public GameObject noteitemhitalkid;

	public GameObject noteitemleadername;

	public GameObject btn_weizhuang;

	public ScrollRect notepanelscrollRect;
	...
	{
		DeleteHighLight(btn_weizhuang, isneedclick: true);
		img_arrow.gameObject.SetActive(value: false);
	}
	...
	public void Next()
	{
		if (iscanclick)
		{
			if (pos == 0)
			{
				notepanelscrollRect.content.localPosition = new Vector2(0f, 208f);
				AddHighLight(noteitemhitalkid);
				AddHighLight(noteitemleadername);
				noteitemhitalkid.GetComponent<NoteItem>().SetHighLightItem(ihl: true);
				noteitemleadername.GetComponent<NoteItem>().SetHighLightItem(ihl: true);
			}
			else if (pos == 3)
			{
				HideBlack();
	...
	private IEnumerator StartHideBlack()
	{
		...
		img_arrow.gameObject.SetActive(value: true);
		DeleteHighLight(noteitemhitalkid);
		DeleteHighLight(noteitemleadername);
		noteitemhitalkid.GetComponent<NoteItem>().SetHighLightItem(ihl: false);
		noteitemleadername.GetComponent<NoteItem>().SetHighLightItem(ihl: false);
		...
		AddHighLight(btn_weizhuang, isneedclick: true);
		MoveArrow();
	}
	...
	private IEnumerator StartHideCourse(bool isactive)
	{
		...
		img_bottom.transform.DOLocalMove(new Vector3(0f, -634f, 0f), 0.2f);
		yield return new WaitForSeconds(0.3f);
		gameManager.player.playerdata.isCourse09 = 1;
		if (isactive)
		{
			gameManager.CanShowSetting(-1);
